...
def build_existing_account_notice_email(user: models.User):
    """Compose the email sent when a registration uses an existing address."""
...
def enqueue_existing_account_notice(
    background_tasks: BackgroundTasks,
    user: models.User,
    *,
    execute_immediately: bool = False,
) -> None:
    """Schedule delivery of the duplicate-registration notice email."""

    settings, message = build_existing_account_notice_email(user)
    log_extra = {"existing_account_user_id": str(user.id)}
    if not settings.host:
        logger.error(
            "Email service misconfigured for existing account notice",
            extra=log_extra,
        )
        return
    if execute_immediately:
        logger.info(
            "Sending existing account notice email immediately",
            extra=log_extra,
        )
        try:
            send_email_via_smtp(
                ...
                logger=logger,
                auth_error=(
                    "SMTP authentication failed when sending existing account notice",
                    "existing_account_notice_auth_failed",
                ),
                ssl_retry=(
                    "SMTP SSL delivery failed for existing account notice, retrying with STARTTLS",
                    "existing_account_notice_ssl_retry",
                ),
                send_failure=(
                    "Failed to send existing account notice email",
            ...
            logger.exception(
                "Immediate existing account notice delivery failed",
                extra=log_extra,
            )
        return
    background_tasks.add_task(
